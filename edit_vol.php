...
if($error==0){
	...
	else{$dcomn = "delete from discs where vid='$vid' ";$dlev = mysql_query($dcomn, $connect); if(!$dlev) {die("Database query failed: ". mysql_error());} 
		foreach ($disArray as $d => $dvalue)
		{if($dvalue!='') 
...
if($error==0){$dcomn = "delete from timing where vid='$vid' ";$dlev = mysql_query($dcomn, $connect); if(!$dlev) {die("Database query failed: ". mysql_error());} 
...
if($error==0){
	...
	else{	$dcomn = "delete from conts where vid='$vid' ";$dlev = mysql_query($dcomn, $connect); if(!$dlev) {die("Database query failed: ". mysql_error());} 
		foreach ($contArray as $c => $cvalue)
		{if($cvalue!='') 
...
if($help == ''){$error=1;?><p align = 'center'><font color="#B30000" face="Verdana" size = "4">Please go back and write a few words about how you can help as a volunteer!</font></p><?}





if($error==0){$dcomn = "delete from experience where vid='$vid' ";$dlev = mysql_query($dcomn, $connect); if(!$dlev) {die("Database query failed: ". mysql_error());} 
		$insert_command = "INSERT INTO experience(natc, intc, onduts, oiduts, expect, exper, training, mapper, sprint, forest, coach, nteam, clubs, si, emit, gps, itex, clubev, localev, natev, hlev, evorg, documents, oskills, 	vid, help) 
		VALUES('$natc', '$intc','$onduts', '$oiduts', '$expect', '$exper','$mapper','$training', '$sprint', '$forest', '$coach', '$nteam', '$clubs', '$si', '$emit', '$gps', '$itex', '$clubev', '$localev', '$natev', '$hlev', 	'$evorg', '$documents', '$oskills', '$vid', '$help')";
   	$sent = mysql_query($insert_command, $connect);
	if(!$sent) {die("Database query failed: ". mysql_error());}}
	...
	if(!empty($dutArray) && isset($dutArray[0])){ 
		...
		{if($dvalue!='') 
if($error==0){$dcomn = "delete from duties where vid='$vid' ";$dlev = mysql_query($dcomn, $connect); if(!$dlev) {die("Database query failed: ". mysql_error());} 
...
<form method="POST" action="edit_vol.php" enctype="multipart/form-data">
...
if($birth>0)
{
	...
	$forest=$row['forest']; $coach=$row['coach']; $nteam=$row['nteam']; $clubs=$row['clubs']; $si=$row['si']; $emit=$row['emit']; $gps=$row['gps']; $itex=$row['itex']; $clubev=$row['clubev']; $localev=$row['localev']; 
	$natev=$row['natev']; $hlev=$row['hlev'];$evorg=$row['evorg']; $documents=$row['documents'];  $oskills=$row['oskills']; $help=$row['help'];
...
{echo 'Welcome back! You may edit your details below </font></b><br><font color="#B30000" face="Verdana" size = "2">Your updated details will be saved when you submit the form </font>';}
?>
</p>
...
<br>
      <table><tr valign="top"><td colspan="2"><font color="#376FA6" face="Verdana" size = "2"><b>O-Experience</b></font></td></tr>
	    <tr><td><font color="#376FA6" face="Verdana" size = "2">Year you started orienteering</td><td>: <font color="#376FA6" face="Verdana" size = "2"><input type="text" name="start" size="4" value="<? if($start>0) echo $start; else echo 'yyyy';?>"><font color="#B30000" face="Verdana" size = "1"> required</font></td</tr>
<tr><td><font color="#376FA6" face="Verdana" size = "2">Your present club (if any) </td><td>: <font color="#376FA6" face="Verdana" size = "2"><input type="text" name="club" value='<? echo $club; ?>' size="20" ></td</tr>
        <tr><td colspan="2"><font color="#376FA6" face="Verdana" size = "2"><u>Experience as Competitor</u> </td></tr>
	<tr><td><font color="#376FA6" face="Verdana" size = "2">Local events </td><td><font color="#376FA6" face="Verdana" size = "2">: <select size="1" name="loc"><option selected value='<? echo $loc; ?>'>--- select ---</option>
					<option value="0" <?=$loc == '0' ? ' selected="selected"' : '';?> >none</option><option value="1" <?=$loc == '1' ? ' selected="selected"' : '';?> >1 - 50</option>
					<option value="2" <?=$loc == '2' ? ' selected="selected"' : '';?> >51 - 100</option>
					<option value="3" <?=$loc == '4' ? ' selected="selected"' : '';?> >over 100</option></select></td></tr>
        <tr><td><font color="#376FA6" face="Verdana" size = "2">National Championships </td><td><font color="#376FA6" face="Verdana" size = "2">: <select size="1" name="nchamp"><option selected value='<? echo $nchamp; ?>'>--- select ---</option>
					<option value="0" <?=$nchamp == '0' ? ' selected="selected"' : '';?> >none</option><option value="1" <?=$nchamp == '1' ? ' selected="selected"' : '';?>>1 - 50</option>
					<option value="2" <?=$nchamp == '2' ? ' selected="selected"' : '';?>>51 - 100</option>
					<option value="3" <?=$nchamp == '3' ? ' selected="selected"' : '';?>>over 100</option></select></td></tr>
        <tr><td><font color="#376FA6" face="Verdana" size = "2">International Competitions <td><font color="#376FA6" face="Verdana" size = "2">: <select size="1" name="icomp"><option selected value='<? echo $icomp; ?>'>--- select ---</option>
					<option value="0" <?=$icomp == '0' ? ' selected="selected"' : '';?>>none</option><option value="1" <?=$icomp == '1' ? ' selected="selected"' : '';?>>1 - 20</option>
					<option value="2" <?=$icomp == '2' ? ' selected="selected"' : '';?>>21 - 50</option>
					<option value="3" <?=$icomp == '3' ? ' selected="selected"' : '';?>>over 50</option></select></td></tr> 
...
<?	$cvar=0; $mvar=0; $ovar=0; $ivar=0; $evar=0; for($x=1;$x<=$n;$x++){if($teach[$x]=="C") $cvar=1;if($teach[$x]=="M") $mvar=1; if($teach[$x]=="O") $ovar=1; if($teach[$x]=="I") $ivar=1;if($teach[$x]=="E") $evar=1;}	?> 
	<br><font color="#376FA6" face="Verdana" size = "1"><input type="checkbox" name="teach[]" value="C" <? if($cvar>0) {?> checked <?}?> > Beginners & children <br><input type="checkbox" name="teach[]" value="M" <? if($mvar>0) {?> checked <?}?>> Teach how to map  <br><input type="checkbox" name="teach[]" value="O" <? if($ovar>0) {?> checked <?}?>> Teach coaching <br><input type="checkbox" name="teach[]" value="I" <? if($ivar>0) {?> checked <?}?>> Teach IT & Timekeeping <br><input type="checkbox" name="teach[]" value="E" <? if($evar>0) {?> checked <?}?> > Teach Event Organising
		...
		<textarea rows="2" cols="30" name="documents" id="documents" value='<? echo $documents; ?>'></textarea></td></tr><tr><td> 
...
<td valign="top" width="17%"> <table><tr valign="top"><td colspan="2"><font color="#376FA6" face="Verdana" size = "2"><b>Additional Information</b></font></td></tr>
	    <tr><td><font color="#376FA6" face="Verdana" size = "2">
<? if($birth>0){?> You may choose one or more different files to upload. In this case, the previous maps you uploaded will be replaced by these ones. <?} else { ?>Skilled in mapping? <br>Upload your "best" maps here...<?}?><br>(At most 3 maps in PDF format)<br><br>
...
<br><br><input type="checkbox" name="ok" >  I have read and understood the above.</font> <font color="#B30000" face="Verdana" size = "1"> Please tick the disclaimer again...</font></td><td> </td></tr>	
<tr><td colspan="4" align="right"><input type="submit" name="editvol" value="Submit my details">
