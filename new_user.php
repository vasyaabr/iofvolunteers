...
if (isset($_POST['regreg'])) 
{
		...
		$email=trim($email); 
		$pword=trim($pword);
	if($uname==''||$email==''||$country==''||$name==''||$pword=='')
	{?>
	...
	{
	$result = mysql_query("SELECT * FROM users WHERE uname='$uname' OR email = '$email' ");
	$num=mysql_num_rows($result);
	if($num!=0)
	{?>
	<p align="center"><font face="Verdana" size = "4" color="#376FA6"><br><br>
	Record with username <? echo $uname; ?> and/or email <? echo $email; ?> already exits. 	<br><br>If you are a new user, go back to choose another username or check your e-mail address. 	<br><br>If you are already registered, Sign In with your username or email and your password.
	</font><p>
	<?}
	else{
	...
	{?>
	<p align="center"><font face="Verdana" size = "4" color="#376FA6"><br><br>
	Record with email <? echo $email; ?> already exists. If you are already registered, Sign In with your username and password. 
	</font><p>
	<?}
	else { 
$birth = 0;
   $insert_command = "INSERT INTO users(name, country, email, uname, pword, birth) VALUES('$name', '$country', '$email', '$uname', '$pword', '$birth')";
   $sent = mysql_query($insert_command, $connect);


      if($sent) {  $id = mysql_insert_id(); $_SESSION['id']=$id; $_SESSION['back']=''; include('welcome.php'); }
	else {?>
	<p align="center"><font face="Verdana" size = "4" color="#376FA6"><br><br>
...
<tr><td colspan="2"><p align="center"><font face="Verdana" color="#FF0000" size="1">All fields are required!</p></td></tr>
...
<tr><td colspan="2"><p align="center"><input type="submit" value="Register me!" name="regreg"></p></td>
